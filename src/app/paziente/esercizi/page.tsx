...
export default function EserciziPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (eserciziData) setEsercizi(eserciziData);

    // Il Set serve per controllare velocemente se un esercizio e' gia' fatto.
    if (logs) {
      setCompletedToday(new Set(logs.map((log) => log.esercizio_id)));
    }
  ...
  async function completeExercise() {
    ...
    await supabase.from('esercizi_log').insert({
        ...
        {esercizi.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {esercizi.map((esercizio) => {
              const isDone = completedToday.has(esercizio.id);

              return (
                <div key={esercizio.id} className="paziente-list-card">
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: '15px', color: '#1E293B' }}>
                      {esercizio.nome}
                    </div>
                    <div style={{ fontSize: '13px', color: '#64748B', marginTop: '2px' }}>
                      ⏱️ {esercizio.durata_minuti} min
                      {esercizio.frequenza && ` · ${esercizio.frequenza}`}
                    </div>
                  </div>
                  ...
                  {isDone ? (
                    ...
                    <button
                      ...
                      onClick={() => startExercise(esercizio)}
                    >
                      ▶ Inizia
                    </button>
                  )}
