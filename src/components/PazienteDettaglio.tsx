...
import styles from '../app/operatore/dashboard/pazienti/[id]/paziente.module.css';
import {
  aggiornaPaziente,
  aggiungiFarmaco,
  creaEsercizio,
  eliminaEsercizio,
  eliminaFarmaco,
} from '../app/operatore/dashboard/pazienti/actions';
...
interface Metriche {
  ...
  umoreLog: UmoreEntry[]; // Ultimi 7 giorni
  farmaciLogSettimana: { data: string; presiCount: number; totaleCount: number }[];
  eserciziLogSettimana: { data: string; completatiCount: number; totaleCount: number }[];
}
    ...
    <div className={styles.page}>
      ...
      {tab === 'dettagli' && (
        <>
          ...
          {/* Storico Farmaci (7 giorni) */}
          <div className={`card ${styles.section}`}>
            <div className="card-header">
              <h3 className="card-title">💊 Storico Farmaci (7 giorni)</h3>
            </div>
            <div className={styles.umoreRow} style={{ padding: '20px', gap: '12px' }}>
              {metriche.farmaciLogSettimana.length === 0 ? (
                <span className={styles.nessunDato}>Nessun dato registrato</span>
              ) : (
          ...
          {/* Storico Esercizi (7 giorni) */}
          <div className={`card ${styles.section}`}>
            <div className="card-header">
              <h3 className="card-title">💪 Storico Esercizi (7 giorni)</h3>
            </div>
            <div className={styles.umoreRow} style={{ padding: '20px', gap: '12px' }}>
              {metriche.eserciziLogSettimana.length === 0 ? (
                <span className={styles.nessunDato}>Nessun dato registrato</span>
              ) : (
                metriche.eserciziLogSettimana.map((d, i) => (
                  <div key={i} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>
                      {d.completatiCount}/{d.totaleCount}
                    </div>
                  </div>
                ))
              )}
